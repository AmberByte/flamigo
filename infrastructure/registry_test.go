...
func TestRegistryAddingStrategyWithPrefix(t *testing.T) {
	registry := NewRegistry[strategies.Context]("test")
	assert.NotPanics(t, func() { registry.Register("test::test", func(ctx strategies.Context) {}) })
}

func TestAddingStrategyWithoutPrefix(t *testing.T) {
	registry := NewRegistry[strategies.Context]("test")
	assert.Panics(t, func() { registry.Register("test", func(ctx strategies.Context) {}) })
}

func TestRegistryCallingStrategy(t *testing.T) {
	registry := NewRegistry[strategies.Context]("test")
	called := false
	registry.Register("test::test", func(ctx strategies.Context) {
		called = true
	})
	ctx := strategies.NewContext(nil, "test::test", map[string]interface{}{})
...
func TestRegistryCallingNonExistentStrategy(t *testing.T) {
	registry := NewRegistry[strategies.Context]("test")
	ctx := strategies.NewContext(nil, "test::test2", map[string]interface{}{})
	result := registry.Use(ctx)
...
func TestRegistryCallingWithWrongPrefix(t *testing.T) {
	registry := NewRegistry[strategies.Context]("test")
	ctx := strategies.NewContext(nil, "test2:test", map[string]interface{}{})
	result := registry.Use(ctx)
